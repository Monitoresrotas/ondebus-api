...
router.get('/', async (req, res) => {
  ...
  // DB: procura uma ligação direta (mesmo trip)
  const fromStops = await q(`SELECT stop_id FROM stops WHERE stop_name ILIKE $1 LIMIT 10`, [`%${origem}%`]);
  const toStops   = await q(`SELECT stop_id FROM stops WHERE stop_name ILIKE $1 LIMIT 10`, [`%${destino}%`]);
  ...
  res.json({ query: { origem, destino }, count: 0, itineraries: [] });
});
